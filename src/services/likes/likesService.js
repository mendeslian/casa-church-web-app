...
export async function createLike(body) {
    const { data } = await axios.post(`${BASE_URL}/likes`, body);
    return data;
}
...
export async function findAllLikes({ page = 1, limit = 10, postId, orderBy = "createdAt", orderDirection = "DESC" } = {}) {
    ...
    const { data } = await axios.get(`${BASE_URL}/likes?${params.toString()}`);
    return data;
}

export async function findLikeById(id) {
    const { data } = await axios.get(`${BASE_URL}/likes/${id}`);
    return data;
}

export async function deleteLike(id) {
    const { data } = await axios.delete(`${BASE_URL}/likes/${id}`);
    return data;
}